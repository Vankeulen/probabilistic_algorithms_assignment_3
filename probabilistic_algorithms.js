...
function primes() {
	...
	var primesUpTo10000 = allPrimes(100000);
	...
	function checkProbalisticPrime(val, k) {
		...
		var actuallyAPrime = primesUpTo10000.includes(val);
		for (var i = 0; i < 1000; i++) {
			// Guess at if the value is prime or not
			var probAPrime = probalisticPrime(val, k);
...
function monteCarloIntegration() {
	...
	var unitRect = { left: -1, right: 1, top: 1, bottom: -1 };
	...
	function dartThrow(f, darts, rect) {
		...
		for (var i = 0; i < darts; i++) {
			...
			var x = rect.left + w * Math.random();
			var y = rect.bottom + h * Math.random();
	...
	var a = dartThrow(yequalsx, 1000000, unitRect);
	...
	var rect2 = {left: 0, right:1, top:1, bottom: 0};
	...
	/*
	var pi = Math.PI;
	// var rectPi = {left: 0, right: Math.PI, bottom:0, top: 1};
